...
export default memo(({ style, tournamentClass, playableTournament, joinedAlready, onPress }: TournamentPlayButtonProps) => {
  ...
  const onClick = () => {
    if (getLeftTime() <= tournamentClass.entryBeforeSecond) {
      errorMessage({message: "Time is over. Try in next round."});
    } else {
      onPress();
    }
  };
